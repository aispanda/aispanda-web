...
AI-114 now requires the RA-002 image-pull preflight helper as well as that staged controller. Select the reusable-assets branch `codex/ai-114-release-image-preflight` through `REUSABLE_AI_ASSETS_ROOT`; it preserves the AI-92 controller and adds the helper. The isolation hook checks both target Cloud Run service agents' effective image download permission before build/promotion. An older owner missing the helper fails closed. Run `node --test tests/release-preflight.test.mjs` with the same root selected to verify this integration. This release-tooling update does not rebuild an application or rewrite prior release receipts.
