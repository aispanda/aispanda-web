...
| `scripts/sync-supportzero-case-workspace.ps1` | Copy the verified fictional SupportZero list workspace into the site | Explicit curated source build → `public/labs/supportzero-case-workspace/` | Static public projection only; no database, runtime API, source map, identity, UUID or local path | Script verifies `index.html`, relative asset paths, fixed destination and public boundary patterns |
