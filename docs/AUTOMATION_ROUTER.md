...
| `scripts/staged-release.sh` + ignored `.staged-release.config` | Thin AI-89 adapter to RA-002 isolated staging and exact-image promotion | Reviewed branch + build-only config + separate stage/prod profiles → immutable receipts | Does not own reusable logic or create infrastructure; requires exact `STAGE`, then separate `DEPLOY` | Runtime/preflight unit tests, RA-002 suite and authenticated staging publication journey |
