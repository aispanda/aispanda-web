...
| EP-11 Community Groups and Discussions | Members create and find durable conversations in moderated, cost-bounded Groups. | In progress | `CONTENT_AUTHORING_REQUIREMENTS.md` | Local Open-to-members vertical slice is in the repo (pages, bounded Group/Discussion discovery, private bookmarks, Group/Discussion/person follows, per-member unread state, private in-app feed, rules/indexes and permission tests). Firestore rules are not published yet. Site-wide search, email/notification preferences, Invite-only, reporting, Events and Learning paths remain documented and deferred. |
