...
const workflow = addConsumerProfileRoutes(JSON.parse(await readFile(sourceUrl, 'utf8')));
const gitAttestationCodes = GIT_ATTESTATION_VIOLATION_CODES.map((code) => `'${code}'`).join(',');
...
validator.parameters.jsCode = replaceOnce(
  ...
  `'MISSING_PERMITTED_ACTION','UNSUPPORTED_ACTION','INVALID_OPERATION_ID','INVALID_BRANCH_FORMAT','LINEAR_BRANCH_MISSING','LINEAR_BRANCH_TASK_MISMATCH','INVALID_REPOSITORY_IDENTITY','CONSUMER_NOT_APPROVED','CONSUMER_REPOSITORY_MISMATCH','ACTION_NOT_APPROVED','CALLER_NOT_APPROVED',${gitAttestationCodes}`,
  'runtime-code set',
);
validator.parameters.jsCode = replaceOnce(
  validator.parameters.jsCode,
...
classifier.parameters.jsCode = replaceOnce(
  ...
  `'MISSING_PERMITTED_ACTION','UNSUPPORTED_ACTION','INVALID_OPERATION_ID','INVALID_BRANCH_FORMAT','LINEAR_BRANCH_MISSING','LINEAR_BRANCH_TASK_MISMATCH','INVALID_REPOSITORY_IDENTITY','CONSUMER_NOT_APPROVED','CONSUMER_REPOSITORY_MISMATCH','ACTION_NOT_APPROVED','CALLER_NOT_APPROVED',${gitAttestationCodes},'CONTROLLER_INVALID_OUTPUT'`,
  'classifier fail set',
);
classifier.parameters.jsCode = replaceOnce(
  classifier.parameters.jsCode,
